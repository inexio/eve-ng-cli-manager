...
var GetLabCmd = &cobra.Command{
	...
	Short: "Returns data about a lab",
	Long: `Returns data about the given Lab.

Lab files have to be given in the following syntax:
	/path/to/labfile.unl
...
The data returned contains:
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		//perform GetLab operation
		labObj, err := client.GetLab(lab)
		if err != nil {
			log.Error().
				Msg("Error during GetLab")
			os.Exit(1)
		}

		//Print the lab object
		err = PrintData(format, labObj, prettified, depth)
		if err != nil {
			log.Error().
...
func init() {
	GetLabCmd.Flags().String("lab", "", "Set the lab-file path (only necessary if not already set via config)")
}
